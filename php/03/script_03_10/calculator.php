<?php # Script 3.10 - calculator.php (5th version after Scripts 3.5, 3.6, 3.8 & 3.9)
$page_title = 'Widget Cost Calculator';
include ('./includes/header.html');

/* This function calculates a total
and then prints the results. 
This function returns the calculated total. */
...
if (isset($_POST['submitted'])) {
	...
	if (is_numeric($_POST['quantity']) && is_numeric($_POST['price'])) {
		...
		if (is_numeric($_POST['tax'])) {
			$total_cost = calculate_total ($_POST['quantity'], $_POST['price'], $_POST['tax']);
		} else {
			$total_cost = calculate_total ($_POST['quantity'], $_POST['price']);
		}
